...
class DGNSSFilter(common.PositionEstimator):
  """
  DGNSSFilter
  ...
  def maybe_update_base_position(self, base_obs):
    ...
    if not self.base_known:
      ...
      spp_pos = spp.single_point_position(base_obs)
      self.base_pos = spp_pos[['x', 'y', 'z']].values[0]
  ...
  def get_single_diffs(self, rover_obs, base_obs, propagate_base=False):
    ...
    """
    rover_toa = common.get_unique_value(rover_obs['time'])
    if propagate_base:
      ...
      assert common.get_unique_value(base_obs['time']) == rover_toa
  ...
  def get_position(self, obs_set):
    baseline_vars = ['baseline_x', 'baseline_y', 'baseline_z']
    baseline = self.get_baseline(obs_set)
    if baseline is None:
      baseline = np.nan * np.empty(3)
    else:
      assert np.all(baseline.index.values == np.array(['x', 'y', 'z']))
      baseline = baseline.values

    return pd.DataFrame(baseline[None, :],
                        columns=baseline_vars,
                        index=pd.Index([obs_set['epoch']], name='time'))
...
class TimeMatchingDGNSSFilter(DGNSSFilter):
  ...
  def _validate_base_observation(self, obs_set):
    ...
    toa = common.get_unique_value(obs_set['base']['time'])
    ...
    if (self.prev_base is not None and
        toa <= common.get_unique_value(self.prev_base['time'])):
