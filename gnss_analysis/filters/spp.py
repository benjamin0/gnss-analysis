...
class SinglePointPosition(common.PositionEstimator):

  def update(self, obs_set):
    return True

  def get_position(self, obs_set):
    if not common.can_compute_position(obs_set['rover']):
      return pd.DataFrame({'x': np.nan,
                           'y': np.nan,
                           'z': np.nan,
                           'clock_offset': np.nan,
                           'converged': False},
      ...
      return single_point_position(obs_set['rover'])
